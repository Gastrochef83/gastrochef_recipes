...
export default function AppLayout() {
  ...
  const [dark, setDark] = useState(() => {
    try { return localStorage.getItem('gc-dark') === '1' } catch { return false }
  })
  ...
  useEffect(() => {
    try {
      localStorage.setItem('gc-dark', dark ? '1' : '0')
    } catch {}
    const root = document.documentElement
    if (dark) root.classList.add('gc-dark')
    else root.classList.remove('gc-dark')
  }, [dark])
