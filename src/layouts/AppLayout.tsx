...
export default function AppLayout() {
  ...
  // ✅ FIX: HashRouter-safe path detection
  const title = useMemo(() => {
    // In HashRouter, pathname can be "/" while the real route is in loc.hash (e.g. "#/recipe?id=...")
    const raw =
            ...
            <div className="gc-side-block">
              ...
              <nav className="gc-nav mt-2">
                <NavLink
                  to="/dashboard"
                  className={({ isActive }) =>
                    cx('gc-nav-item', isActive && 'is-active')
                  }
                >
                  Dashboard
                </NavLink>
