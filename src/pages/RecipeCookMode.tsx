...
// Note: Cook Mode is now fully synced with DB.
// We do NOT merge drafts from localStorage anymore to avoid stale/duplicated lines.
...
export default function RecipeCookMode() {
  ...
  const load = async (recipeId: string) => {
    ...
    try {
      ...
      // load lines (robust across schema variants)
      let l: any[] = []
      // Prefer the newer schema (position/notes) if present, otherwise fall back to legacy (sort_order/note)
      {
      ...
      // Normalize to CookMode Line shape
      ...
      setCheckedLines((sess as any)?.checkedLines ?? {})
  ...
  useEffect(() => {
    if (!id) return
    saveCookSession(id, { servings, checkedSteps: checked, checkedLines, timers })
  }, [id, servings, checked, checkedLines, timers])

  // Re-sync from DB when tab becomes active (helps when user edits in RecipeEditor, then returns here)
  useEffect(() => {
    if (!id) return
    const onFocus = () => {
      load(id).catch(() => {})
    ...
    const onVis = () => {
      if (document.visibilityState === 'visible') load(id).catch(() => {})
    }
  ...
  const toggleLine = (lineId: string) => setCheckedLines((p) => ({ ...p, [lineId]: !p[lineId] }))
  ...
  const ingredientChecklist = useMemo(() => {
    // Keep group headers, show ingredient lines with scaled qty.
    // (No subrecipe expansion in Day 4; stays fast and avoids logic changes.)
    return (lines ?? []).map((l) => {
      if (l.line_type === 'group') {
        return {
          kind: 'group' as const,
          id: l.id,
          title: (l.group_title ?? '').trim() || 'Group',
        }
      }
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-[min(640px,92vw)]">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <button className="gc-btn gc-btn-ghost" type="button" onClick={refreshFromDb}>
                  Sync
                </button>
            ...
            {/* ===== INGREDIENT CHECKLIST (SCALED) — ALWAYS VISIBLE ===== */}
            {prepList.length > 0 && (
              <div className="mt-5 rounded-2xl border border-neutral-200 bg-white p-4">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <div className="gc-label">INGREDIENTS — CHECKLIST (SCALED)</div>
                    <div className="text-xs text-neutral-500">Tap to mark prepared · stays saved for this recipe.</div>
                  </div>
                ...
                <div className="mt-3 grid gap-2">
                  {ingredientChecklist
                    ...
                    .map((x: any, idx: number) => {
                      if (x.kind === 'group') {
                        return (
                          <div key={`g-${x.id}-${idx}`} className="mt-2 text-xs font-extrabold tracking-wide text-neutral-700">
                            {x.title}
                          </div>
                        )
                      }
                      const done = !!checkedLines[x.id]
                      return (
                        <button
                          key={`l-${x.id}`}
                          type="button"
                          onClick={() => toggleLine(x.id)}
                          className={`flex w-full items-center justify-between gap-3 rounded-xl border px-3 py-2 text-left transition ${
                            done ? 'border-emerald-200 bg-emerald-50' : 'border-neutral-200 bg-neutral-50 hover:bg-white'
                          }`}
                        >
                          <div className="flex min-w-0 items-start gap-3">
                            <div className={`mt-0.5 h-5 w-5 rounded-md border ${done ? 'bg-emerald-500 border-emerald-500' : 'bg-white border-neutral-300'}`} />
                            <div className="min-w-0">
                              <div className={`truncate font-semibold ${done ? 'text-emerald-900 line-through' : 'text-neutral-900'}`}>{x.label}</div>
                              {x.note ? <div className="truncate text-xs text-neutral-500">{x.note}</div> : null}
                            </div>
                          </div>
                          <div className={`shrink-0 font-bold tabular-nums ${done ? 'text-emerald-900' : 'text-neutral-900'}`}>
                            {fmtQty(x.qty)} {x.unit}
                          </div>
                        </button>
                      )
                ...
                {ingredientChecklist.filter((x: any) => x.kind === 'line').length > 12 ? (
                  <div className="mt-2 text-xs text-neutral-500">
                    Showing 12 of {ingredientChecklist.filter((x: any) => x.kind === 'line').length} items.
                  </div>
                ) : null}
              </div>
            )}
