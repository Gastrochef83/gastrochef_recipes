...
const addLineLocal = useCallback(async () => {
  ...
  // Kitchen Spec Sheet print (A4) — execution focused.
  const printKitchenSheetNow = useCallback(() => {
    if (!id) return
    const url = `#/print-kitchen?id=${encodeURIComponent(id)}&autoprint=1`
    window.open(url, '_blank', 'noopener,noreferrer')
  }, [id])
              ...
              >
                <div>
                  ...
                  <div className="gc-hint" style={{ marginTop: 6 }}>
                    Restaurant-ready printing: clean A4 output with no overflow.
                  </div>
                  <div className="gc-hint" style={{ marginTop: 10 }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                  ...
                  <button className="gc-btn gc-btn-soft" type="button" onClick={printKitchenSheetNow} disabled={!id}>
                    Kitchen Spec Sheet
                  </button>
                  <button
                    className="gc-btn gc-btn-ghost"
                    type="button"
                    onClick={() =>
                      id ? window.open(`#/print-kitchen?id=${encodeURIComponent(id)}`, '_blank', 'noopener,noreferrer') : null
                    }
                    disabled={!id}
                  >
                    Open Kitchen Page
