...
function isDraftLine(l: any) {
  return typeof l?.id === 'string' && l.id.startsWith('tmp_')
}
...
// Meta fields
  ...
  // ---------- Debounced lines save (ABSOLUTE SAFE) ----------
  const [savingLines, setSavingLines] = useState(false)
  const linesSaveTimer = useRef<number | null>(null)
  const savingLinesOffTimer = useRef<number | null>(null)
  const deletedLineIdsRef = useRef<string[]>([])

  const loadLinesFromDB = useCallback(async () => {
    if (!id) return
    const { data: l, error: lErr } = await supabase
      .from('recipe_lines')
      .select(
  ...
  const setSavingLinesSmooth = useCallback((v: boolean) => {
    if (savingLinesOffTimer.current) {
      window.clearTimeout(savingLinesOffTimer.current)
      savingLinesOffTimer.current = null
    }
    if (v) {
      setSavingLines(true)
    } else {
      // prevent blinking
      savingLinesOffTimer.current = window.setTimeout(() => {
        setSavingLines(false)
      }, 700)
    }
  }, [])

  const saveLinesNow = useCallback(async (overrideLines?: any[]) => {
  if (!recipeRef.current?.id) return false

  const kitchenId = recipeRef.current?.kitchen_id ?? k.kitchenId ?? null

  // Kitchen/session not ready yet: retry silently later
  if (!kitchenId) {
  ...
  try {
    setSavingLinesSmooth(true)

    const current = ((overrideLines ?? linesRef.current) || []) as any[]
    const toDelete = (deletedLineIdsRef.current || []).filter(Boolean)

    if (toDelete.length) {
      const { error: dErr } = await supabase.from('recipe_lines').delete().in('id', toDelete)
      if (dErr) {
        if (linesSaveTimer.current) window.clearTimeout(linesSaveTimer.current)
        linesSaveTimer.current = window.setTimeout(() => {
          saveLinesNow()
    ...
    const clean = current.filter((l) => !l?.__deleted)

    const payload = clean.map((l) => ({
      id: isDraftLine(l) ? undefined : l.id,
      ...
      qty: Number(l.qty ?? 0),
      unit: l.unit ?? 'g',
      yield_percent: Number(l.yield_percent ?? 100),
      notes: l.notes ?? null,
    ...
    // Force DB to return representation so we can verify it actually persisted (RLS / triggers / defaults)
    const { data, error } = await supabase
      .from('recipe_lines')
      // onConflict explicit keeps behavior stable across PostgREST versions
    ...
    // Verify: for a real successful write, DB must return rows for this recipe
    const wroteForRecipe = data.some((r) => r.recipe_id === recipeRef.current!.id)
    if (!wroteForRecipe) {
      if (linesSaveTimer.current) window.clearTimeout(linesSaveTimer.current)
      linesSaveTimer.current = window.setTimeout(() => {
        saveLinesNow()
    ...
    await loadLinesFromDB()
    return true
  } finally {
    setSavingLinesSmooth(false)
  }
}, [k.kitchenId, loadLinesFromDB, setSavingLinesSmooth])
  ...
  const deleteLineLocal = useCallback((lineId: string) => {
    if (!lineId) return
    // track DB deletes only for real ids
    if (!isDraftLine({ id: lineId })) {
      deletedLineIdsRef.current = Array.from(new Set([...(deletedLineIdsRef.current || []), lineId]))
    }
    const next = (linesRef.current || []).map((l: any) => (l.id === lineId ? { ...l, __deleted: true } : l))
    setLinesSafe(next.filter((l: any) => !l.__deleted))
    scheduleLinesSave()
  }, [scheduleLinesSave])
...
const addLineLocal = useCallback(async () => {
    ...
    if (addType === 'ingredient') {
      ...
      setErr(null)
      const nextLines = [...linesRef.current, newL]
      linesRef.current = nextLines as any
      setLinesSafe(nextLines)
      const ok = await saveLinesNow(nextLines)
      showToast(ok ? 'Line added & saved.' : 'Line added (kept locally — saving soon).')
      return
    }
    ...
    if (addType === 'subrecipe') {
      ...
      setLinesSafe(nextLines)
      scheduleLinesSave()
      showToast('Subrecipe line added (saving…).')
      return
    }
