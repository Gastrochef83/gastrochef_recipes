...
const k = useKitchen()
  ...
  // ---------- HARD Draft Guard (prevent disappearing ingredients) ----------
  const draftCreatedAt = useRef<Record<string, number>>({})

  const markDraftCreated = useCallback((id:string)=>{
    draftCreatedAt.current[id] = Date.now()
  },[])

  const isDraftTooFresh = (id:string)=>{
    const t = draftCreatedAt.current[id]
    if(!t) return false
    return Date.now() - t < 2000 // wait 2s before autosave
  }
  // ---------- Debounced lines auto-save (ERP: save every step) ----------

  const isLineReadyToSave = useCallback((l: Line) => {
    if (l.line_type === 'group') return true
    if (isDraftTooFresh(l.id)) return false
    if (l.line_type === 'ingredient') return Boolean(l.ingredient_id)
    if (l.line_type === 'subrecipe') return Boolean(l.sub_recipe_id)
    return true
  }, [])
  ...
  const linesSignature = useCallback((arr: Line[]) => {
    const stable = normalizePositions(arr.filter(isLineReadyToSave)).map((l) => ({
      id: l.id,
      position: l.position,
      line_type: l.line_type,
      ingredient_id: l.ingredient_id,
  ...
  const saveLinesNowSafe = useCallback(async () => {
    if (!id) return
    const readyLines = (linesRef.current || []).filter(isLineReadyToSave)
    if (readyLines.length === 0) return

    const sig = linesSignature(readyLines)
    if (sig === lastSavedSig.current) return

    setSavingLines(true)
    try {
      await saveLinesNow()
      lastSavedSig.current = linesSignature((linesRef.current || []).filter(isLineReadyToSave))
    } finally {
      setSavingLines(false)
    }
  }, [id, saveLinesNow, linesSignature, isLineReadyToSave])
